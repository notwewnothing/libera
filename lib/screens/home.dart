  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          _HeroCarousel(future: trendingCards, count: _heroCount),
...
class _HeroCarousel extends StatefulWidget {
  final Future<List<MediaCardData>> future;
  final int count;

  const _HeroCarousel({required this.future, required this.count});
...
class _HeroCarouselState extends State<_HeroCarousel> {
  final PageController _controller = PageController(initialPage: 4000);
  int _page = 0;
        ...
        children: [
          FutureBuilder<List<MediaCardData>>(
            ...
            builder: (context, snapshot) {
              ...
              final items = (snapshot.data ?? []).take(widget.count).toList();
              if (items.isEmpty) {
                return Center(
                  child: Text(
                    snapshot.hasError ? "Error: ${snapshot.error}" : "No Data",
                  ...
                  Positioned(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        IgnorePointer(child: _dots(items.length)),
                      ],
                    ),
                  ),
                ],
  ...
  Widget _meta(MediaCardData item) {
    final label = [
      item.typeLabel,
      item.genreLabel,
  ...
  Widget _buttons(MediaCardData item) {
    return Row(
      mainAxisAlignment: MainAxisAlignment.center,
      children: [
        Pressable(
          ...
          child: Container(
            ...
            child: const Row(
              mainAxisAlignment: MainAxisAlignment.center,
              children: [
                Icon(Icons.info_rounded, color: Colors.black, size: 28),
                SizedBox(width: 4),
                Text(
                  "More Info",
                  style: TextStyle(
