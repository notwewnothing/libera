...
/// Inline WebView embed player (Android / iOS / macOS), with progress tracking
/// and episode-change detection via the page's `LiberaProgress` JS channel.
///
/// This file imports `webview_flutter`, so it is selected only on platforms
/// that have `dart:io` (see inline_webview_player.dart) — web uses the stub.
class InlineWebViewPlayer extends StatefulWidget {
  final String title;
  ...
  @override
  State<InlineWebViewPlayer> createState() => _InlineWebViewPlayerState();
}

class _InlineWebViewPlayerState extends State<InlineWebViewPlayer> {
  ...
  void dispose() {
    ...
    setSystemUIModeSafe(SystemUiMode.manual, overlays: SystemUiOverlay.values);
    super.dispose();
  }
